...
class ImageUploader {
  ...
  public function getImages() {
      ...
      while(false !==($file = readdir($imageDir))) {
          ...
          if(file_exists(THUMBNAIL_DIR . '/' . $file)) {
                ...
                $images[] = basename(UPLOAD_DIR) . '/' . $file;
          }
      }
  ...
  private function _createThumbnailMain($savePath, $width, $height) {
    switch($this->_imageType) {
      ...
      switch($this->_imageType) {
        case IMAGETYPE_GIF:
            imagegif($thumbImage, 'thumbnail/' . $this->_imageFileName);
            break;
        case IMAGETYPE_JPEG:
            imagejpeg($thumbImage, 'thumbnail/' . $this->_imageFileName);
            break;
        case IMAGETYPE_PNG:
            imagepng($thumbImage, 'thumbnail/' . $this->_imageFileName);
            break;
      }
  }
  ...
  private function _save($ext) {
    ...
    // $savePath =  . '/' . $this->_imageFileName;
    $savePath = 'upload/' . $this->_imageFileName;
